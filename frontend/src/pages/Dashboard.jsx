...
const fetchAlerts = useCallback(() => {
  if (!token || !selectedOrgId) {
    setAlerts([]);
    return () => {}; // no unsubscribe if no org selected
  }

  // Reference to the Firestore collection
  const alertsRef = collection(db, "incidents");

  // Query alerts for selected organization, newest first
  const q = query(
    alertsRef,
...
//   return () => {
//     if (unsubscribe) unsubscribe(); // clean up subscription on unmount
//   };
// }, [fetchAlerts]);
  ...
  const unsubscribe = onSnapshot(
    ...
    (snapshot) => {
      ...
      // 🔔 Optional: notify for new critical alerts
      const latest = alertList[0];
      if (latest && latest.severity === "Critical" && !latest.is_fixed) {
        if (Notification.permission === "granted") {
          new Notification("🚨 Critical Alert Detected", {
            body: latest.description || "A critical incident has occurred!",
          });
        }
